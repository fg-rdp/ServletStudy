...
//    }
    public PageBean<Customer> findAll(int pc, int pr) {
        try {
            ...
            sql = "select * from (select t.*, ROWNUM rn from t_customer t order by name) where rn between ? and ?";
...
//    }
    ...
    public PageBean<Customer> query(Customer customer, int pc, int pr) {
        ...
        try {
            ...
            String sql = "select * from (select t.*, ROWNUM rn from t_customer t " + whereSql + ") where rn between ? and ? ";
            params.add((pc - 1) * pr);
            params.add(pr);
